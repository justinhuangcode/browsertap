...
pub fn verify_token(token: &str, secret: &[u8]) -> Result<TokenPayload, TokenError> {
    let (encoded_payload, encoded_sig) = token.split_once('.').ok_or(TokenError::InvalidFormat)?;
...
pub fn secret_from_hex(hex: &str) -> Result<Vec<u8>, TokenError> {
    (0..hex.len())
        .step_by(2)
        .map(|i| u8::from_str_radix(&hex[i..i + 2], 16).map_err(|_| TokenError::InvalidFormat))
        .collect()
}
