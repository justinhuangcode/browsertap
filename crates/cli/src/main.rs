...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Screenshot {
            ...
            let result = client
                .screenshot(&session, selector.as_deref(), quality)
                .await?;
            if result.ok {
                if let Some(data) = &result.data
                    && let Some(b64) = data.get("base64").and_then(|v| v.as_str())
                {
                    let bytes =
                ...
                match nav {
                    Ok(r) if r.ok => {
                        ...
                        let errors: Vec<_> = console
                            .iter()
                            .filter(|e| {
                                matches!(e.level, browsertap_shared::protocol::ConsoleLevel::Error)
                            })
                            .collect();
        ...
        Commands::Console {
            ...
            let filtered: Vec<_> = events
                ...
                .filter(|e| {
                    level
                        .as_ref()
                        .is_none_or(|l| format!("{:?}", e.level).to_lowercase() == l.to_lowercase())
                })
                .collect();
